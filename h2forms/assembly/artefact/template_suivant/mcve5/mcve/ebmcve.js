...
var MCVE_VERSION = "modellwerkstatt mcve 5.0 (Version 64)";
...
// fullUrl = "https://10.233.82.88:3000/upload";
var picName = '';
...
/* ------------------------------------------------------------------------------------------------ */
function someFunctionName(args){
	mLog('someFunctionName', 'Args param is ' + args);
	
	status = '(no status found)';
    if ('response_code' in args){
    	var code = args['response_code']
    	status = 'EB 5 - ' + code + ' for ' + picName;
    	
    	if (code == '200') {
    		imgPath = 'https://' + tomcatServer + '/static/' + picName;
	        $('img').src = imgPath;
	        mLog('someFunctionName', 'EB 5 Load path to image ' + imgPath);
    	}
    
    } else if ('body' in args) {
    	status = args['status'] + ' / ' + args['body'];
        imgPath = 'https://' + tomcatServer + '/static/' + args['body']
        $('img').src = imgPath;
        mLog('someFunctionName', 'Load path to image ' + imgPath);
        
    }
    mLog('someFunctionName', 'Status is  ' + status);
...
function mCameraPicTaken(cbData){
    if ('imageUri' in cbData) {
        ...
        try {
           ...
           EB.Network.uploadFile(uploadfileProps, someFunctionName);
